...
def admin_pricing_new():
    ...
    if request.method == "POST":
        ...
        db.session.add(PricingPlan(platform_type=pt, monthly_posts=posts, monthly_fee=fee, sort_order=sort_order, stripe_price_id=stripe_price_id))
        db.session.commit()
...
def admin_pricing_edit(plan_id: int):
    ...
    if request.method == "POST":
        ...
        plan.stripe_price_id = request.form.get("stripe_price_id", "").strip()
        db.session.commit()
