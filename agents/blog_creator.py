...
【段落の書き方】見出し（##・###）の前後、段落と段落の間、箇条書きブロックの前後には必ず空行（改行2つ）を入れること。単文を並べる場合も文と文の間に空行を入れ、読みやすく区切ること。

【タイトル必須ルール】
- タイトルは1文で完結すること
- 「〇〇——△△」「〇〇：△△」「〇〇｜△△」のような「前半——後半」形式の2部構成・サブタイトル付きは絶対禁止
- ダッシュ（——・―・─）で2つに分割するタイトルは禁止
- 短く端的な1フレーズにまとめること（例：「プロが教える素人感とプロの分岐点」）"""
...
class BlogCreatorAgent(BaseAgent):
    def _build_message(self, data: dict) -> str:
        ...
        business_section = f"\n\n## 企業の事業内容・サービス概要\n{business_description}" if business_description else ""
